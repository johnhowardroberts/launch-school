...
# Yes, this does alter the values of the keys specified in the method. Adds 42
# to their 'age' and changes the value of 'gender'

# This is because Ruby is passing the object id by reference, and there is
# no reassignment. The object id of the original values of the keys are being
# altered by the mess_with_demographics method.

# full explanation given in the solution:
...
# object id's in locally scoped (private to the method) variables
# (named per the method definition's parameter list).
#
# So Spot's demo_hash starts off pointing to the munsters hash. His program
# could wind up replacing that with some other object id and then the
# family's data would be safe.
#
